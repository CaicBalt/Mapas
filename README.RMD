...
huamachucoensis <- plants %>% 
  ...
  write_xlsx(huamachucoensis,path = tempfile(fileext = "huamachucoensis.xlsx/"), col_names = TRUE, format_headers = TRUE, use_zip64 = FALSE)
...
## Creamos archivos shapefile de cada especie
```{r}
Resumen <- plants %>% group_by(ESPECIES) %>% 
  summarise(Resumen, by = n())
```
